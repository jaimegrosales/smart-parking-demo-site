"""
Summer Sub-Model — LightGBM Production Version
===============================================
Improvements for unseen-data prediction:
  ...
  3. Statistical lookup features: zone+hour+dow mean/std from training data
     (these work on unseen data because they're derived from training history)
  4. New time features: minute, day_of_year_sin/cos, week_of_year_sin/cos
  5. LightGBM + 100-trial Optuna search (minimizes MAE)
  6. Event columns dropped (summer model doesn't use them)

OUTPUTS:
  best_summer_lgbm_eval.pkl            — trained on 80%, for scoring
  best_summer_lgbm_production.pkl      — trained on 100%, for deployment
  summer_stat_lookup.pkl               — lookup table (needed at inference time)
  summer_stat_lookup_production.pkl    — full-data lookup for deployment
"""
...
def add_time_features(df):
    """All features derivable from Timestamp alone — safe for unseen future data."""
    df = df.copy()
...
def build_stat_lookup(df):
    """
    Computes mean & std of availability per (Zone, hour, Day of Week).
    Built from training data only — applied at inference via merge.
    """
    lookup = (
...
# *** UPDATE THIS PATH ***
data = pd.read_csv(
    "/Volumes/Untitled/smart-parking-ML-25-26/ensemble_ML/summer_model/TEST_24-ONLYsummer_data.csv"
)

data = add_time_features(data)
data = data.drop(columns=EVENT_COLUMNS, errors='ignore')
#data = data[data['month'].isin([5, 6, 7])].reset_index(drop=True)
print(f"Summer rows: {len(data):,}", flush=True)

# ---------------------------------------------------------------------------
# Temporal 80/20 split (CSV is pre-sorted chronologically)
# ---------------------------------------------------------------------------
...
# Build lookup from training data only — prevents test leakage
stat_lookup = build_stat_lookup(train_df)
joblib.dump(stat_lookup, "summer_stat_lookup.pkl")
...
print_scores("Summer LightGBM — Eval (80/20 Temporal)", y_test, eval_m.predict(X_test), elapsed)
joblib.dump(eval_m, "best_summer_lgbm_eval.pkl")
print("Saved: best_summer_lgbm_eval.pkl", flush=True)

# ---------------------------------------------------------------------------
# Production model — 100% data, same best params
# ---------------------------------------------------------------------------
print("Training production model (100% data)...", flush=True)
full_lookup = build_stat_lookup(data)
joblib.dump(full_lookup, "summer_stat_lookup_production.pkl")
...
joblib.dump(prod_m, "best_summer_lgbm_production.pkl")
print("Saved: best_summer_lgbm_production.pkl", flush=True)
print("Saved: summer_stat_lookup_production.pkl")
print("\nDeploy: best_summer_lgbm_production.pkl + summer_stat_lookup_production.pkl")
