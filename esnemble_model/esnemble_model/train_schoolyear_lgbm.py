"""
School Year Sub-Model — LightGBM Production Version
====================================================
Improvements for unseen-data prediction:
  ...
  6. Event columns dropped; summer months excluded

OUTPUTS:
  best_schoolyear_lgbm_eval.pkl            — trained on 80%, for scoring
  best_schoolyear_lgbm_production.pkl      — trained on 100%, for deployment
  schoolyear_stat_lookup.pkl               — lookup table (needed at inference time)
  schoolyear_stat_lookup_production.pkl    — full-data lookup for deployment
"""
...
# *** UPDATE THIS PATH ***
data = pd.read_csv(
    "/Volumes/Untitled/smart-parking-ML-25-26/ensemble_ML/schoolYear_model/SY_WITH_WKNDS-NO-EVENTS_FUTURE_cyclic_1-model-small.csv"
)

data = add_time_features(data)
data = data.drop(columns=EVENT_COLUMNS, errors='ignore')

# Exclude summer months and event rows — matches ensemble routing
# data = data[~data['month'].isin([5, 6, 7])].reset_index(drop=True)
print(f"School year rows: {len(data):,}", flush=True)
...
joblib.dump(stat_lookup, "schoolyear_stat_lookup.pkl")
...
print_scores("School Year LightGBM — Eval (80/20 Temporal)", y_test, eval_m.predict(X_test), elapsed)
joblib.dump(eval_m, "best_schoolyear_lgbm_eval.pkl")
print("Saved: best_schoolyear_lgbm_eval.pkl", flush=True)
...
joblib.dump(full_lookup, "schoolyear_stat_lookup_production.pkl")
...
joblib.dump(prod_m, "best_schoolyear_lgbm_production.pkl")
print("Saved: best_schoolyear_lgbm_production.pkl", flush=True)
print("Saved: schoolyear_stat_lookup_production.pkl")
print("\nDeploy: best_schoolyear_lgbm_production.pkl + schoolyear_stat_lookup_production.pkl")
